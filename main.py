...
# - Main code -
                      ...
                      class metodos:
                          def whois():
                          domain=input("%s%sDigite o dominio que deseja consultar:%s "%(start,B,r));
cls()
...
case _:
print("%s[%s!%s%s] %s%s%s"%(n,R,r,n,R,str(rspj["message"]),r));
print("%sSe o site começa com https:// remova essa parte.%s"%(start,r));
input("\n%s%sAperte a tecla %s%sEnter%s%s para voltar.%s"%(start,B,r,G,r,B,r))
def cep():
    _cep=input("%s%sDigite o cep que deseja localizar:%s "%(start,B,r));
cls()
cep=[]
    for remove in _cep:
    if remove.isnumeric:
                                                      ...
                                                      for key, value in DICT.items():
                                                          if isinstance(value, dict):
                                                              ...
                                                              metodos.nlist(value)
                                                                  ...
                                                                  def cnpj():
                                                                  _cnpj=input("%s%sDigite o CNPJ que deseja consultar%s: "%(start,B,r));
cls()
cnpj=[]
     for remove in _cnpj:
...
input("\n%s%sAperte a tecla %s%sEnter%s%s para voltar.%s"%(start,B,r,G,r,B,r))
         ...
         if rsp.status_code!=200:
                                                             print("%s[%s!%s%s] %s%s%s"%(n,R,r,n,R,str(rspj["message"]),r))
                                                                     ...
                                                                     input("\n%s%sAperte a tecla %s%sEnter%s%s para voltar.%s"%(start,B,r,G,r,B,r))
                                                                     def ddd():
                                                                         ddd=input("%s%sDigite o DDD que deseja consultar%s: "%(start,B,r));
cls()
...
input("\n%s[%s+%s%s%s] %sAperte a tecla %s%sEnter%s%s para voltar.%s"%(start,B,r,G,r,B,r))
def ip():
    ip=input("%s%sDigite o IP que deseja localizar%s: "%(start,B,r));
cls()
    ...
    rspj=rsp.json()
         if rspj["status"]=="success":
             ...
             input("\n%s%sAperte a tecla %s%sEnter%s%s para voltar.%s"%(start,B,r,G,r,B,r))
                 ...
                 print("%s[%s!%s%s] %sOcorreu um erro na requisição. Status:  %s%s%s"%(n,R,r,n,R,r,rspj["status"],r));
input("%s%sAperte a tecla %s%sEnter%s%s para voltar.%s"%(start,B,r,G,r,B,r))
